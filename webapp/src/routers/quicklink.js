          ...
          request(
            ...
            (err, res3, body) => {
                if (res3.body.status != true) {
                    ...
                    quicklinkService.saveQuicklinkBilling(user_id,mer_authen_level,member_code, carrier_id, billing_no, branch_id, total, img_url).then(function(data) {})
                    ...
                    for(j=0;j<items.length;j++) {
                        ...
                        quicklinkService.checkTrackingBillingItem(billing_no,sender_name,sender_phone,sender_address,source,trackingItem,zipcodeItem,parcelTypeItem,sizePriceItem,codValueItem,sizeIdItem).then(function(dataBillItem) {})
                    }
                    res.json({
                        status:'success',
