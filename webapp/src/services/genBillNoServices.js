...
module.exports = {
    ...
    checkPrice: (code, size_name) => {
        var sql = "SELECT size_id,parcel_price FROM size_info where location_zone=? and alias_size= ?"
    ...
    checkZipcode: (zipcode) => {
        var sql = "SELECT p.PROVINCE_NAME FROM postinfo_zipcodes z "+
    ...
    checkTrackingBillItem: (tracking) => {
        var  sql = "SELECT tracking FROM billing_item WHERE tracking =? AND (source is null or source ='KEYIN')"
        var data=[tracking];
    ...
    getReceiptMember: (member_code, branch_id) => {
        var sql = "SELECT b.member_code,b.billing_date,b.billing_no,b.branch_id,bInfo.branch_name,b.total,b.timestamp,"+
    ...
    getType: (bill) => {
        var sql = "SELECT bItem.parcel_type,count(bItem.parcel_type) as cType, sum(cod_value) as sumCOD "+
    ...
    getMemberBillType: (member_code, branch_id) => {
        var sql = "SELECT bi.parcel_type,count(bi.parcel_type) as cType, sum(cod_value) as sumCOD " +
        "FROM billing b " +
    ...
    savePrintBill: (billing_no, total, first_name, member_code, branch_id, branch_name, timestamp, tracking, size_price, parcel_type, alias_size, receiver_name, province_name, zipcode,cod_value,count) => {
        var status = 'U'
        var sql = "INSERT INTO printbill(billing_no, total, first_name, member_code, branch_id, branch_name, timestamp, tracking, size_price, parcel_type,alias_size, receiver_name, province_name, zipcode,status,cod_value,count) "+
        "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"
        var data=[billing_no, total, first_name, member_code, branch_id, branch_name, timestamp, tracking, size_price, parcel_type,alias_size, receiver_name, province_name, zipcode,status,cod_value,count];
        return new Promise(function (resolve, reject) {  
            connection.query(sql, data, (err, results) => {})
        })
    }
